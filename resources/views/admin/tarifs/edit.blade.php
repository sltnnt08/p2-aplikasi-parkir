...
@section('title', 'Edit Tarif - Admin')

@section('content')
<div class="space-y-6">
    <!-- Header -->
    <div>
        <h1 class="text-3xl font-semibold tracking-tight text-[#191c1e]">Edit Tarif</h1>
        <p class="text-sm text-[#64748b] mt-1">Perbarui data tarif parkir</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-lg shadow-[0px_12px_32px_rgba(30,41,59,0.06)] p-6 lg:p-8 max-w-2xl">
        <form method="POST" action="{{ route('admin.tarifs.update', $tarif->id_tarif) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                >
                    ...
                    @foreach($areas as $area)
                        <option value="{{ $area->id_area }}" {{ (string) old('id_area', $tarif->id_area) === (string) $area->id_area ? 'selected' : '' }}>
                            {{ $area->nama_area }}
                        </option>
                    @endforeach
                </select>
